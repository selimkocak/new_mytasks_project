# backend\api\views\task_views_part1.py kodlarım
...
class TaskViewSetPart1(viewsets.ModelViewSet):
    ...
    def get_queryset(self, status_filter=None):
        ...
        all_tasks = list(owned_tasks) + list(team_tasks)  # Convert QuerySets to lists and concatenate them
        return all_tasks
    ...
    def create(self, request, *args, **kwargs):
        ...
        task.team_members.add(user)  # Görevi oluşturan kullanıcıyı Viewers alanına ekleyin.
        task.save()
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.get_queryset(status_filter)

        if order_by:
            queryset = sorted(queryset, key=lambda task: getattr(task, order_by.strip('-')), reverse=order_by.startswith('-'))  # Update this line
